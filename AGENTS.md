...
- Prefer existing Webflow elements, neutral root/track/slide attributes, and component-owned design classes for custom Swiper addons. Use the adapter's opt-in `structure` mode only when the installed version supports it; temporary upstream classes and layout must restore on teardown. See [slider authoring](docs/sliders.md) for version requirements and shared-vendor integration.
